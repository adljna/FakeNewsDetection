...
logR_pipeline = Pipeline(
    [("LogRCV", FeatureSelection.countV), ("LogR_clf", LogisticRegression())]
)
...
svm_pipeline = Pipeline(
    [("svmCV", FeatureSelection.countV), ("svm_clf", svm.LinearSVC())]
)
...
svm_pipeline_ngram = Pipeline(
    [("svm_tfidf", FeatureSelection.tfidf_ngram), ("svm_clf", svm.LinearSVC())]
)
...
# Linear SVM
parameters = {
    "svm_tfidf__ngram_range": [(1, 1), (1, 2), (1, 3), (1, 4), (1, 5)],
...
random_forest_final = Pipeline(
    [
        (
            "rf_tfidf",
            TfidfVectorizer(
                stop_words="english", ngram_range=(1, 3), use_idf=True, smooth_idf=True
            ),
        ),
...
logR_pipeline_final = Pipeline(
    [
        ...
        (
            "LogR_tfidf",
            TfidfVectorizer(
                stop_words="english", ngram_range=(1, 5), use_idf=True, smooth_idf=False
            ),
        ),
        ("LogR_clf", LogisticRegression(penalty="l2", C=1)),
    ]
...
def plot_learing_curve(pipeline, title):
    ...
    train_sizes, train_scores, test_scores = learning_curve(
        pl, X, y, n_jobs=-1, cv=cv, train_sizes=np.linspace(0.1, 1.0, 5), verbose=0
    )
...
def plot_PR_curve(classifier):
    ...
    plt.xlim([0.0, 1.0])
    plt.title(
        "2-class Random Forest Precision-Recall curve: AP={0:0.2f}".format(
            average_precision
        )
    )
...
def show_most_informative_features(model, vect, clf, text=None, n=50):
    ...
    coefs = sorted(zip(tvec[0], vectorizer.get_feature_names()), reverse=True)
